...
def concat_datasets_argo(mems_dict, recon_output_dir, init_date , fin_date):
...
def concat_mirrored_datasets(mems_dict,recon_output_dir, init_date , fin_date):
    ...
    for ens, mem_list in mems_dict.items():
        ...
        for n_member, member in enumerate(mem_list):          
            recon_dir = f"{recon_output_dir}/{ens}/{member}"  
            print(member)
            recon_path = f"{recon_dir}/recon_pCO2residual_{ens}_{member}_mon_1x1_{init_date}_{fin_date}_mirrored.zarr"
            kwargs = dict(chunks={'time':-1})
            truth = xr.open_zarr(recon_path, consolidated=True)["pCO2_truth"]
            recon = xr.open_zarr(recon_path, consolidated=True)["pCO2_recon_unseen"]
...
def concat_datasets_argo_clim(mems_dict, recon_output_dir, init_date , fin_date):
    
    datasets_ens = []
    for ens, mem_list in mems_dict.items():
        datasets_member = []
        for n_member, member in enumerate(mem_list):          
            recon_dir = f"{recon_output_dir}/{(ens + "_argo_clim")}/{member}"    
            recon_path = f"{recon_dir}/recon_pCO2_{(ens + "_argo_clim")}_{member}_mon_1x1_{init_date}_{fin_date}.zarr"
            kwargs = dict(chunks={'time':-1})
            truth = xr.open_zarr(recon_path, consolidated=True)["pCO2_truth"]
            recon = xr.open_zarr(recon_path, consolidated=True)["pCO2_recon_unseen"]
