...
def calc_recon_pco2_argo(regridded_members_dir, pco2_recon_dir, selected_mems_dict, init_date, fin_date, owner_name=None):
    ...
    for ens, mem_list in run_selected_mems_dict.items():
        ...
        for member in mem_list:
            ...
            ### Path to reconstruction (ML output from notebook 02), where pCO2-residual was reconstructed
            pCO2R_path = f"{pco2_recon_dir}/{ens_argo}/{member}/recon_pCO2residual_{ens_argo}_{member}_mon_1x1_{init_date}_{fin_date}.zarr"
            print('pCO2R path:',pCO2R_path)

            ### Path to save calculated pCO2 (reconstructed pCO2-residual PLUS pCO2-T: Total pCO2 =  pCO2-residual + pCO2-T)
            file_out = f"{pco2_recon_dir}/{ens_argo}/{member}/recon_pCO2_{ens_argo}_{member}_mon_1x1_{init_date}_{fin_date}.zarr" # change this to just pco2
            print('save path:',file_out)
...
def calc_recon_pco2_argo_clim(regridded_members_dir, pco2_recon_dir, selected_mems_dict, init_date, fin_date, owner_name=None):
